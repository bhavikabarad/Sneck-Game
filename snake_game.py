...
class SnakeGame:
    ...
    def draw_snake_segment(self, surface, x: int, y: int, is_head: bool = False):
        ...
        if is_head:
            ...
            pygame.draw.rect(surface, self.BLUE, segment_rect)
            # Draw eyes
            eye_size = 3
            ...
            # Body segments
            pygame.draw.rect(surface, self.DARK_PINK, segment_rect)
    ...
    def draw_ui(self):
        ...
        if self.game_over:
            ...
            final_score_text = self.font.render(f"Final Score: {self.score}", True, self.DARK_PINK)
            restart_text = self.small_font.render("Show 'UP' gesture to restart", True, self.BLUE)
